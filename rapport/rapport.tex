...
\subsection{Filtre de Tchebychev}
\subsubsection{Choix des valeurs de composants}
...
\subsubsection{La technologie micro ruban}
\textbf{Les lignes de transmission inductives\\}


Dans cette partie on cherche à remplacer les inductances série du filtre par des lignes de transmissions qui apportent le même effet capacitif. La figure \ref{fig:probleme_ligne_inductive} illustre les deux schémas que nous utiliserons.
...
On précise que $Z_c$ est l'impédance caractéristique de la ligne de transmission, $l$ est la longueur de la ligne, $L$ la valeur de l'inductance et $Z_0$ la charge. L'impédance vue en entrée de la ligne de transmission est donné par la formule :

\begin{equation}
	Z_{vue} = Z_c\ \frac{Z_0 + j Z_c tan(\beta l)}{Z_c + j Z_0 tan(\beta l)}
\end{equation}

Dans le cas où l'on choisit une ligne de transmission avec une impédance caractéristique identique à l'impédance de charge on a $Z_c=Z_0$ et donc :

\begin{equation}
	Z_{vue} = Z_0 + j Z_c tan(\beta l)
\end{equation}

L'impédance vue en entrée du montage avec l'inductance idéale L est donne par 
...
\begin{equation}
	Z_0 + j 2\pi f L = Z_0 + jZ_c\frac{2 \pi f}{c} l
\end{equation}
...
\textbf{Les lignes de transmission capacitives\\}

Dans cette partie on cherche à remplacer les condensateurs parallèles du filtre par des lignes de transmissions qui apportent le même effet capacitif. La figure \ref{fig:probleme_ligne_capacitive} illustre les deux schémas pour cette étude.
...
\textbf{Limites de validité\\}

\subsubsection{Passage des composants discrets à des lignes de transmissions}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=10cm]{../3synthPBas/elliptique/solution_admittance.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=10cm]{../3synthPBas/elliptique/}
	\caption{Topologie admittance du filtre elliptique en éléments localisés (b) et de son équivalent en technologie micro-ruban}
