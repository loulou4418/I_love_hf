...
Comme nous l'avons vu dans les parties précédentes, des lignes de transmission peuvent s'apparenter à des effets capacitifs ou inductifs sous certaines conditions. Ces conditions sont aux nombres de deux. La première est sur l'impédance de la ligne de transmission $Z_c$ tandis que la deuxième est lié à la constante de phase $\beta$ et plus particulièrement la fréquence.\\
...
Dans le cas où l'on cherche apparenter une bobine série à une ligne de transmission, son impédance caractéristique doit être très supérieur à $Z_0$ soit $Z_c >> Z_0$. Dans ces conditions l'impédance $Z_c$ s'écrit :
...
\begin{figure}[H]
	...
	\begin{circuitikz}[scale=0.85]
		 \draw (0,0)
		 to[mstline, mstlinelen=2, l=line$Z_c \beta$,*-] (4,0) to[R,l=$Z_0$] (4,-2)
		 node[ground]{};
	\end{circuitikz}
...
\end{figure}
La figure \ref{fig:res_parallele} représente un résonateur LC parallèle à droite et  un stub demis onde CO à gauche. Ils ramènent tous deux un circuit ouvert à la fréquence de résonance. En effet, l'admittance du premier est très proche de l'impédance du résonateur série est d'exprime :
\begin{equation}
	Y(\omega)=j\Big(C\omega-\frac{1}{L\omega}\Big)
\end{equation}  
De nouveau on obtient en admittance cette fois: 
\begin{equation}
Im(\underline{Y_{equ}}(\omega)) = 0 \text{ donc }\underline{Y_{equ}}(\omega_0)=0, \quad \omega_0 =\frac{1}{\sqrt{LC}}
\end{equation}
Le résonateur série ramène donc un admittance nulle soit un circuit ouvert. La démonstration de l'impédance ramenée du stub demie onde parallèle est la même que pour la ligne $\lambda /2$ série : $Z_{vue} = Z_0$. La terminaison du stub étant un circuit ouvert, l'impédance ramenée est infinie. Nous venons donc de monter qu'un résonateur LC parallèle peut être transposé en un stub demie onde ouvert. 
